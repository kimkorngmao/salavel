...
@section('title') Edit Enrolling @endsection

@section('content')
<div class="p-6 rounded-lg border border-gray-200">
    <div class="flex items-center justify-between border-b pb-5">
        <h1 class="text-lg text-gray-900 font-semibold">Edit Enrollment</h1>
    ...
    <form action="{{ route('enrollments.update', $enrollment->id) }}" method="POST">
            ...
            <div class="mt-2.5">
                <select name="course_id" id="course_id" class="block w-full rounded-md border-0 outline-none px-3.5 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                    ...
                    @foreach ($courses as $course)
                        <option data-price="{{ $course->price }}" value="{{ $course->id }}" {{ $enrollment->course_id == $course->id ? 'selected' : '' }}>{{ $course->name }}</option>
                    @endforeach
                </select>
            </div>
            @error('course_id')
        ...
        <div class="mt-5">
            <p id="coursePrice" class="text-sm text-gray-700">Course Price: <span id="priceDisplay">{{ $enrollment->course->price }}</span> USD</p>
        </div>
